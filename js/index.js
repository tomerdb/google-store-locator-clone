...
const searchLocationsNear = (stores) => {
    ...
    stores.map((store, index) => {
        let latlng = new google.maps.LatLng(
            ...
            createMarker(latlng, name, address, index + 1, openStatusText, phone, location);
        })
        map.fitBounds(bounds);
    }
    
    const createMarker = (latlng, name, address, storeNumber, openStatusText, phone, location) => {
        let html = `
    ...
    const setStoresList = (stores) => {
        ...
        stores.map((store,index) => {
            ...
            <div class="store-info-container">
                        <div class="store-address"><span>${store.addressLines[0]}</span><span>${store.addressLines[1]}</span></div>
